...
- Student profile creation, editing, soft deactivation, and face biometric enrollment (5-sample quality-guided average).
...
The application is built using Python and dependencies defined in [`requirements.txt`](file:///c:/SURAJ/AI_Attendance_System/requirements.txt):
...
1. **UI Presentation Layer** ([`app/ui/`](file:///c:/SURAJ/AI_Attendance_System/app/ui)): Manages user interactions, event handling, view switching, and canvas rendering. UI frames never call raw SQL directly.
2. **Application Service Layer** ([`app/auth/`](file:///c:/SURAJ/AI_Attendance_System/app/auth), [`app/students/`](file:///c:/SURAJ/AI_Attendance_System/app/students), [`app/attendance/`](file:///c:/SURAJ/AI_Attendance_System/app/attendance), [`app/reports/`](file:///c:/SURAJ/AI_Attendance_System/app/reports), [`app/analytics/`](file:///c:/SURAJ/AI_Attendance_System/app/analytics)): Implements core business logic, RBAC session validation, filtering, exports, and metric aggregations.
3. **AI Engine Layer** ([`app/ai/`](file:///c:/SURAJ/AI_Attendance_System/app/ai)): Orchestrates video frame acquisition (`FrameProvider`), YuNet face detection, SFace 128D embedding extraction, quality checking, and Cosine Similarity matching.
4. **Repository Data Access Layer** ([`app/database/repository.py`](file:///c:/SURAJ/AI_Attendance_System/app/database/repository.py)): Encapsulates all SQL statements using 100% parameterized placeholders (`?`) inside atomic transaction blocks.
5. **Persistence Layer** ([`app/database/connection.py`](file:///c:/SURAJ/AI_Attendance_System/app/database/connection.py)): Manages connection handles to local SQLite file `data/attendance.db` with enforced referential integrity (`PRAGMA foreign_keys = ON;`).
...
### 1. Authentication & RBAC Module (`app/auth`)
...
### 2. Student Management Module (`app/students`)
...
### 3. Face Registration Module (`app/students/registration.py` & `app/ai/enrollment.py`)
...
### 4. AI Attendance Engine Module (`app/ai/pipeline.py` & `app/attendance`)
...
### 5. Management Dashboard Module (`app/dashboard`)
...
### 6. Reports & Export Module (`app/reports`)
...
### 7. Visual Analytics Module (`app/analytics`)
...
### 8. Packaging & Deployment Module (`build_app.py` & `ai_attendance_system.spec`)
...
- **Processing**: PyInstaller bundling with `sys._MEIPASS` path resolution.
...
Raw plaintext passwords and hash strings are never written to log files or displayed on UI screens. When a user logs out, `SessionManager.clear_session()` immediately destroys the in-memory RAM session.
...
2. **Camera Preview**: Live camera preview opens in `StudentFaceRegistrationWindow`.
...
The **Management Dashboard** (`DashboardViewFrame`) provides a real-time summary view of daily attendance metrics:
...
The **Reports Module** (`ReportsViewFrame`) offers searching, manual attendance status corrections, and multi-format report exports:
...
The **Visual Analytics Module** (`AnalyticsViewFrame`) embeds a 2x2 grid panel of Matplotlib statistical charts rendered directly on a Tkinter canvas (`FigureCanvasTkAgg`):
...
- **`MainWindow` Shell**: Unified application layout featuring topbar header, status indicators (`AI Status: MODEL AVAILABLE`), logged-in user role badge, and left navigation sidebar.
...
- **Dialogs & Modals**: `ConfirmationDialog` modal popups prevent accidental student deactivations or logout actions.
- **Empty State Widgets**: `EmptyStateWidget` provides friendly feedback when search filters return zero matching database records.
...
1. **`CameraFrameProvider`**: Standard production provider interfacing local USB webcams or integrated laptop cameras via OpenCV `cv2.VideoCapture(index)`.
2. **`ImageFrameProvider`**: Development/testing provider loading single static image files for deterministic AI testing.
3. **`VideoFrameProvider`**: Testing provider reading pre-recorded video files.
4. **`MobileCameraFrameProvider`**: Testing adapter connecting mobile smartphone cameras via USB tethering and DroidCam IP stream endpoints (`http://127.0.0.1:4747/video`).
...
- **Database Tests** (`test_stage1_database.py` - 20 tests): Schema creation, FK enforcement, CRUD, transactions.
- **Auth Tests** (`test_stage2_auth.py` - 16 tests): Password hashing, login, RBAC roles, session clearance.
- **Student Tests** (`test_stage3_students.py` - 14 tests): Profile validation, unique student code checks, deactivation.
- **AI Engine Tests** (`test_stage4_ai.py` - 16 tests): YuNet detection, SFace 128D embeddings, Cosine threshold 0.363 matching.
- **Registration Tests** (`test_stage5_registration.py` - 9 tests): 5-sample capturing, quality sharpness check, vector averaging.
- **Attendance Engine Tests** (`test_stage6_attendance.py` - 9 tests): Live pipeline, 10s cooldown, SQLite duplicate block.
- **Dashboard Tests** (`test_stage7_dashboard.py` - 5 tests): Summary metric calculations and recent activity feeds.
- **Reports & Export Tests** (`test_stage8_reports.py` - 9 tests): Multi-criteria search, CSV export, OpenPyXL Excel export.
- **Analytics Tests** (`test_stage9_analytics.py` - 7 tests): Trend aggregations, donut distribution, performance bands.
- **UI Polish Tests** (`test_stage10_ui_polish.py` - 5 tests): CustomTkinter view rendering and layout hierarchy.
- **Hardening Tests** (`test_stage11_hardening.py` - 8 tests): Boundary conditions, corrupted inputs, SQL injection attempts.
- **Packaging Tests** (`test_stage12_packaging.py` - 6 tests): PyInstaller frozen paths, `sys._MEIPASS` resolution, launcher scripts.
- **Mobile Camera Adapter Tests** (`test_mobile_camera_provider.py` - 5 tests): Socket stream handling and adapter fallback.
- **Stage 0 Baseline Tests** (`test_stage0.py` - 2 tests): Environment setup and dependency version checks.
...
A full 100% project repository audit was conducted across all 20 production modules in `app/`.
...
*(Refer to [`PROJECT_REPORT_SCREENSHOT_PLAN.md`](file:///c:/SURAJ/AI_Attendance_System/docs/PROJECT_REPORT_SCREENSHOT_PLAN.md) for full visual layout specifications.)*

1. **User Login Interface**: `[SCREENSHOT PLACEHOLDER #1: User Login Screen (LoginWindow)]`
2. **First-Run Setup Window**: `[SCREENSHOT PLACEHOLDER #2: First-Run Admin Setup Window]`
3. **Management Dashboard**: `[SCREENSHOT PLACEHOLDER #3: Management Dashboard (DashboardViewFrame)]`
4. **Student Management View**: `[SCREENSHOT PLACEHOLDER #4: Student Management Interface (StudentManagementFrame)]`
...
7. **Attendance Reports View**: `[SCREENSHOT PLACEHOLDER #7: Attendance Reports View (ReportsViewFrame)]`
8. **Exported Excel Report**: `[SCREENSHOT PLACEHOLDER #8: Exported Excel Workbook (.xlsx) Report Output]`
9. **Visual Analytics Grid**: `[SCREENSHOT PLACEHOLDER #9: Visual Analytics 2x2 Chart Grid (AnalyticsViewFrame)]`
...
1. **Python Software Foundation**: Python 3.13.14 Documentation. Website: `https://www.python.org/`
2. **OpenCV DNN Module**: Open Source Computer Vision Library & OpenCV Zoo. ONNX Models: YuNet (`face_detection_yunet_2023mar.onnx`) & SFace (`face_recognition_sface_2021dec.onnx`). Website: `https://opencv.org/`
3. **CustomTkinter GUI Library**: Modern Dark-Themed Tkinter UI Framework by Tom Schimansky. Website: `https://customtkinter.tomschimansky.com/`
4. **SQLite Database Consortium**: SQLite 3 Self-Contained Relational Database Engine. Website: `https://www.sqlite.org/`
5. **Matplotlib Development Team**: Matplotlib 2D Plotting Library for Python & Tkinter Integration (`FigureCanvasTkAgg`). Website: `https://matplotlib.org/`
6. **OpenPyXL Project**: OpenPyXL — Python Library to Read/Write Excel 2010 xlsx/xlsm Files. Website: `https://openpyxl.readthedocs.io/`
7. **Pandas Project**: Pandas Data Analysis and Manipulation Tool. Website: `https://pandas.pydata.org/`
8. **PyInstaller Development Team**: PyInstaller Standalone Application Freezer. Website: `https://pyinstaller.org/`
9. **Pytest Development Team**: Pytest Framework for Python Testing. Website: `https://docs.pytest.org/`
